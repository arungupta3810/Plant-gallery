...
└── backend/                 SELF-CONTAINED NestJS API (movable to own repo)
    ├── prisma/schema.prisma Data model (Users, Plants, Orders, Content, Lead, Subscription)
    ├── prisma/seed.ts       Starter data (the only hardcoded data)
    └── src/                 Modules: auth, plants, orders, wishlist, content, contact, admin
```
